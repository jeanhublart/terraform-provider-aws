...
//
// This function can be used in conjuction with resourceRecordSetEqual to determine
// cases where record _identifiers_ match but the entirety of the record is not equal.
// In these scenarios the record can use an Upsert change in the batch, rather than
// one Delete and one Create change.
//
...
func resourceRecordSetIdentifiersEqual(s1, s2 awstypes.ResourceRecordSet) bool {
	return fwtypes.RecordNamesSemanticallyEqual(aws.ToString(s1.Name), aws.ToString(s2.Name)) &&
		s1.Type == s2.Type &&
...
func aliasTargetEqual(s1, s2 *awstypes.AliasTarget) bool {
	...
	return aws.ToString(s1.DNSName) == aws.ToString(s2.DNSName) &&
